...
import org.healthbeauty.service.OrderService;
...
public class CustomerController {
    ...
    public String personalInfo(Model userModel,HttpServletRequest request){
        HttpSession session=request.getSession();
        User user;
        Integer userId;
        user=(User) session.getAttribute("user");
...
//           orderItemExample.or().andOrderidEqualTo(order.getOrderid());
           orderItemList=orderService.selectList(order.getOrderid());
...
//           goodsExample.or().andGoodsidIn(goodsIdList);
           goodsList=goodsService.selectList(goodsIdList);
